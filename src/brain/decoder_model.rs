...
use crate::{
    brain::{
        bpe::{BpeTokenizer, EOS_ID, TokenizerKind}, fixer::fix_json_syntax, model::GenerationResult, samples::{Action, CardinalDir, TrainingStage, WorldContext}, train::MAX_SEQ_LEN
    },
...
use crate::brain::flash_attn::attention::{TransformerBlock, TransformerBlockConfig, causal_mask};
...
impl YumonDecBrainConfig {
    pub fn init<B: Backend<Device = WgpuDevice>>(&self, device: &B::Device) -> YumonDecBrain<B> {
        ...
        let blocks = (0..self.n_layers)
            .map(|_| TransformerBlockConfig::new(self.embed_dim, self.ff_dim, self.attn_heads).init(device))
            .collect();
...
impl<B: Backend<Device = WgpuDevice>> YumonDecBrain<B> {
    ...
    pub fn forward(
        ...
        let pad_mask = tokens.equal_elem(PAD_TOKEN as u32);
        let cmask = causal_mask::<B>(seq_len, &device);

        let mut x = self.dropout.forward(
            self.embedding.forward(tokens_cl)
        );
    ...
    pub fn generate_unmasked_parsed(
        ...
        // ── Decode tokens → string (skip BOS, prompt, and separator)
    ...
    pub async fn generate_unmasked_parsed_async(
        ...
        let prompt_len = if seed_text.is_empty() { 1 } else { 1 + tokenizer.encode(seed_text).len() };
        let raw_output = tokenizer.decode(&dec_ids[prompt_len..]);
    ...
    pub fn save(&self, directory: &str, tokenizer: &TokenizerKind, metadata: &BrainDecMetadata) -> Result<()> {
    ...
    pub fn load(directory: &str, device: &B::Device) -> Result<(Self, TokenizerKind, YumonDecBrainConfig)> {
        ...
        let metadata: BrainDecMetadata = serde_json::from_str(&meta_json)?;
    ...
    pub async fn load_from_bytes(
        ...
        tokenizer_bytes: &[u8],
        device: &B::Device,
    ) -> Result<(Self, TokenizerKind, YumonDecBrainConfig)> {
        ...
        let metadata: BrainDecMetadata = serde_json::from_str(metadata_json)?;
        ...
        // let model: YumonDecBrain<Backend> = YumonDecBrain::new(&Default::default());
