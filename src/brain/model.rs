...
/// Yumon Brain — transformer language model with multi-head output.
///
/// Architecture:
///   Input per timestep: token_embedding (EMBED_DIM) + positional_embedding (EMBED_DIM)
///   → TransformerEncoder(N_LAYERS) → LayerNorm → Dropout
///   ├─ token_head:       Linear(vocab_size)    — next token prediction   [batch, seq, vocab]
///   └─ (from last token position only):
///      ├─ yumon_emote_head: Linear(EMOTE_CLASSES) — emotional state      [batch, EMOTE_CLASSES]
///      ├─ action_head:      Linear(8)              — action choice        [batch, 8]
///      └─ motion_head:      Linear(8)              — movement direction   [batch, 8]
///
/// Context is now encoded directly into input_ids as special tokens:
///   [<OBS:dir:dist>, <RES:dir:dist>, <BLD:dir:dist>, <BOS>, t0, t1, …]
///
/// Action classes (ordinal):
///   0=Travel, 1=Build, 2=Collect, 3=Speak, 4=Emote, 5=Idle, 6=Wait, 7=Reserved
///
/// Motion classes (ordinal):
...
pub const ACTION_CLASSES: usize = 8;
pub const MOTION_CLASSES: usize = 8;
...
impl YumonBrainConfig {
    pub fn init<B: Backend>(&self, device: &B::Device) -> YumonBrain<B> {
        ...
        YumonBrain {
            ...
            token_head:       LinearConfig::new(EMBED_DIM, self.vocab_size).init(device),
            yumon_emote_head: LinearConfig::new(EMBED_DIM, EMOTE_CLASSES).init(device),
            action_head:      LinearConfig::new(EMBED_DIM, ACTION_CLASSES).init(device),
            motion_head:      LinearConfig::new(EMBED_DIM, MOTION_CLASSES).init(device),
        }
    }
}
...
impl<B: Backend> YumonBrain<B> {
    ...
    ///
    /// Returns:
    ///   token_logits  — [batch, seq, vocab]
    ///   emote_logits  — [batch, EMOTE_CLASSES]
    ///   action_logits — [batch, ACTION_CLASSES]
    ///   motion_logits — [batch, MOTION_CLASSES]
    ///   ()            — reserved for future recurrent state
    pub fn forward(
        &self,
        tokens: Tensor<B, 2, Int>,
        _prev_state: Option<()>,
        ...
        // 3. Padding mask — true = pad position, ignored by attention
        ...
        // 5. Token head — full sequence
        let token_logits = self.token_head.forward(x.clone());

        // 6. All single-token heads fire from the last non-pad position.
        //    During training this is always seq_len-1 (samples are right-padded).
        //    At inference the generate() loop handles variable lengths.
        let last = x
            .slice([0..batch, seq_len - 1..seq_len])
            .reshape([batch, EMBED_DIM]);

        let emote_logits  = self.yumon_emote_head.forward(last.clone());
        let action_logits = self.action_head.forward(last.clone());
        let motion_logits = self.motion_head.forward(last);

        (token_logits, emote_logits, action_logits, motion_logits, ())
    }
    ...
    pub fn generate(
        ...
        let mut rng = rand::thread_rng();
        let mut last_emote_logits:  Option<Vec<f32>> = None;
        let mut last_action_logits: Option<Vec<f32>> = None;
        let mut last_motion_logits: Option<Vec<f32>> = None;
        ...
        for _ in 0..max_tokens {
            ...
            let (token_logits, emote_logits, action_logits, motion_logits, _) =
                self.forward(tokens_t, None);
            ...
            last_emote_logits  = Some(emote_logits.to_data().to_vec::<f32>().unwrap());
            last_action_logits = Some(action_logits.to_data().to_vec::<f32>().unwrap());
            last_motion_logits = Some(motion_logits.to_data().to_vec::<f32>().unwrap());
        ...
        let yumon_emote_idx  = last_emote_logits.as_deref().map(argmax).unwrap_or(4);
        let yumon_action_idx = last_action_logits.as_deref().map(argmax).unwrap_or(5); // default: Idle
        let yumon_motion_idx = last_motion_logits.as_deref().map(argmax).unwrap_or(0); // default: N

        GenerationResult { reply, yumon_emote_idx, yumon_action_idx, yumon_motion_idx }
    }
    ...
    pub fn load(directory: &str, device: &B::Device) -> Result<(Self, TokenizerKind)> {
        ...
        let record   = recorder
            ...
            .map_err(|e| anyhow::anyhow!("load: {e:?}"))?;
        let model    = YumonBrainConfig::new(metadata.vocab_size)
            .init::<B>(device)
            .load_record(record);
    ...
    pub fn load_app(directory: &str, device: &B::Device) -> Result<(Self, TokenizerKind)> {
        ...
        let tokenizer = TokenizerKind::Bpe(
            BpeTokenizer::load("../../yumon-pet/yumon_bpe")?
        );
...
pub struct GenerationResult {
    pub reply:           String,
    pub yumon_emote_idx: usize,
    /// Index into Action enum: 0=Travel,1=Build,2=Collect,3=Speak,4=Emote,5=Idle,6=Wait,7=Reserved
    pub yumon_action_idx: usize,
    /// Index into direction: 0=N,1=NE,2=E,3=SE,4=S,5=SW,6=W,7=NW
    pub yumon_motion_idx: usize,
}
