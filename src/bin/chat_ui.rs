...
use yumon_pet::{
    brain::{
        model::{GenerationResult, YUMON_SCHEMA}, moe_model::YumonMoeBrain, samples::{TrainingStage, WorldContext}, train::MAX_SEQ_LEN, xlstm_model::YumonXLstmBrain
...
#[cfg(target_os = "windows")]
    ...
    let brain_cp = "checkpoints/brain/256h_16l_4a_32len_b8_Moe_e4_k1_Language".to_string();
...
fn ui(f: &mut ratatui::Frame, app: &AppState) {
    ...
    let items: Vec<ListItem> = app.messages.iter().rev().take(25).rev().map(|m| {
        match m {
            Message::User(s) => {
