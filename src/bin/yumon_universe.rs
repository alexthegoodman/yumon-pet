...
impl Yumon {
    ...
    fn build_prompt(&mut self, others: &[Building]) -> String {
        ...
        serde_json::to_string_pretty(&serde_json::json!({
            ...
            "command":        "".to_string(), // TODO: ensure the last command remains the current command until replaced
            "message":        text,
    ...
    // ── Brain thread ──────────────────────────────────────────────────────────
    {
        ...
        std::thread::spawn(move || {
            ...
            let (brain, tokenizer, config) =
                match YumonBrain::<Wgpu>::load("checkpoints/brain/128h_2l_2a_220len_6e", &device) {
                    Ok(m) => m,
